...
func ExampleFlagSet() {
	start := func(args []string) {
		// A real program (not an example) would use zflag.ExitOnError.
		fs := zflag.NewFlagSet("start", zflag.ContinueOnError)
	...
	stop := func(args []string) {
		fs := zflag.NewFlagSet("stop", zflag.ContinueOnError)
